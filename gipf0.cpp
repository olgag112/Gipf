...
int main()
{
	...
	while (cin >> commands)
	{
		if (commands == "LOAD_GAME_BOARD")
		{
			...
			for (int i = 0; i < 2 * size + 1; i++)
			{
				board[i] = new char[4 * size + 3];
				if (i == 0 || i == 2 * size) {
					for (int j = 0; j < 4 * size + 3; j++) {
						if (j >= size && j <= 3 * size + 2) {
							board[i][j++] = '+';
							board[i][j] = ' ';
						}
						else {
					...
					for (int j = 0; j < 4 * size + 3; j++)		//4*size + 3
					{
						board[i][j] = row[j];
						//board[i][j] = getchar();
			...
			//cout << edge_indexes[edge_size];			// wywala przy edge_index = 15 i2
		...
		else if (commands == "DO_MOVE")				//DO MOVE
		{
			...
			for (int i = 0; i < 2 * size + 1; i++)
			{
				for (int j = 0; j < 4 * size + 3; j++)
				{
					if (i == end_cords[0] && j == end_cords[1])
					{
		...
		else if (commands == "PRINT_GAME_BOARD")			// PRINT GAME BOARD
		{
			for (int i = 0; i < 2 * size - 1; i++)
			{
				...
				{
					//if (board[i][j] == ' ' || board[i][j] == '_' || board[i][j] == 'W' || board[i][j] == 'B' || board[i][j] == '+')
						cout << board[i][j];
					//else
						//continue;
